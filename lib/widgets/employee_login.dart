...
class EmployeeLogin extends StatefulWidget {
  ...
  @override
  State<EmployeeLogin> createState() => _EmployeeLogin();
}
...
class _EmployeeLogin extends State<EmployeeLogin> {
  ...
  Future<String> getUserBasicInfo(String loginId) async {
    final response = await http.post(
        Uri.parse(ApiConfig.BASE_URL + ApiConfig.GET_USER_INFO + loginId));
  ...
  Future<String> getAuthOTP(
    ...
    String url = ApiConfig.BASE_URL +
        ApiConfig.GET_AUTH_OTP_NUM +
        loginId +
        ApiConfig.GET_AUTH_OTP_NAME +
        userName +
        ApiConfig.GET_AUTH_OTP_CONTACT +
        userContactNo;
    ...
    if (response.statusCode == 200) {
      ...
      await prefs.setString(ApiConfig.PTEF_KEY_USER_TYPE, "Employee");
      print("new list size: ${data}");
      return data;
    } else {
      throw Exception('Failed to load User Info');
